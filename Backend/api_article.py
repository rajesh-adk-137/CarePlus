...
async def enforce_policy(user, action, resource):
    opa_url = os.getenv("OPA_SERVER_URL")
    query = {
        "input": {
            "user": {"role": user.role},
            "action": action,
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{opa_url}/v1/data/authz/allow", json=query)
    if response.status_code == 200:
        result = response.json()
        if not result.get("result", False):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied")
    else:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="OPA policy check failed")
...
# @app.post("/get_all/")
# async def get_all(url: str = Form(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
#     await enforce_policy(user, "read", "article")
#     title, content, comments, likes = scrape_article(url)
...
# @app.post("/get_answer/")
# async def get_answer_route(url: str = Form(...), question: str = Form(...), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
#     await enforce_policy(user, "query", "article")
#     title, content, comments, likes = scrape_article(url)
#     if title:
#         text = '"' + content + '"'
